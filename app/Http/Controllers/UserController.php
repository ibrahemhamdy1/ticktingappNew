...
class UserController extends Controller {
    ...
    public function __construct(UserRequest $request, User $model)
    {
        ...
        $this->middleware('role:admin|salesManager|supportManager',['only' => ['create','store','edit','update']]);
...
//           return $rows;
    ...
    {
//        $this->request->merge(['type' =>1]);
        $insert = $this->model->create($this->request->all());
        $insert->roles()->sync($this->request->input('role'));
        if ($insert) {
//Insert Blog Gallary

            if ($this->request->ajax())
                return response()->json(array('status' => 'true', 'message' => "Add user Done Sucessfully"));
            return redirect()->back()->with('success', "Add user Done Sucessfully");
...
//$update->update($this->request->all());
//$update->roles()->sync($this->request->role);
